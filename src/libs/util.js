...
export const moment = (data) => {
  // 超过七天显示日期
  // isBefore检查一个 moment 是否在另一个 moment 之前
  if (dayjs(data).isBefore(dayjs().subtract(7, 'days'))) {
    return dayjs(data).format('YYYY-MM-DD')
  } else {
    // 1小时前，xx小时前，x天前
    return dayjs(data).locale('zh-cn').from(dayjs())
  }
}
 ...
 * arr: 整个菜单
 * node: 点击的节点
 * 写这种算法时候，应该先从一级考虑，然后在二级三级，就是从边缘到一般
 */
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (currentNode.nodeKey === node.nodeKey) {
      ...
      if (currentNode.children && currentNode.children.length > 0) {
        ...
        // 当前循环中是否有该节点(这个判断是嵌套判断,只有当全部为true后，下面代码才会执行)
